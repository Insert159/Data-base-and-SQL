...
SELECT users.id, users.firstname, users.lastname, profiles.photo_id, profiles.hometown 
FROM users u
JOIN cities c ON u.city_id = c.id
JOIN (
    SELECT user_id, filename as avatar 
    FROM media 
...
SELECT u.id, u.first_name, u.last_name, c.city, m.filename as avatar 
FROM users u 
JOIN cities c ON u.city_id = c.id 
LEFT JOIN (
